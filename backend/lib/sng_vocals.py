...
Some SNG-to-XML toolchains only handle instrumental arrangements, so official DLC
...
log = logging.getLogger("feedback_studio.lib.sng_vocals")
...
def _decrypt_sng(data: bytes, platform: str) -> bytes:
    ...
    decrypted = aes_ctr_decrypt(key, iv, encrypted)
