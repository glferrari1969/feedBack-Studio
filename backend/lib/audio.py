...
def _vgmstream_cmd(resolution_notes: list[str] | None = None) -> str | None:
    ...
    Resolution order:
      ...
      3. `vgmstream-cli` on PATH
        4. Backend-local tool install path (`tools/vgmstream/vgmstream-cli*`),
            where setup_tools.py places the downloaded binary.
        5. Repo-local build outputs (autotools `.libs/`, CMake `build/cli/`,
