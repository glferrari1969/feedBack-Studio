...
log = logging.getLogger("feedback_studio.lib.sloppak_convert")
...
from tones import annotate_tone_block_with_vst, extract_tones_for_song
from audio import find_wem_files, _vgmstream_cmd, _ffmpeg_cmd, _ffmpeg_wav_to_ogg

try:
    from wem_decode import convert_wem_to_ogg as _convert_wem_to_ogg_py
except Exception:
    _convert_wem_to_ogg_py = None
...
_EXCLUDED_ARRANGEMENTS = {"vocals", "showlights", "jvocals"}
...
def _parse_toolkit_authoring_metadata(extracted_dir: Path) -> dict[str, str]:
    """Read CDLC toolkit.version and map authoring fields for the manifest.

    Expected lines (case-insensitive):
      - Package Author: <value>  -> metadata.charter
      - Package Version: <value> -> metadata.version
      - Package Comment: <value> -> metadata.notes
    """
    path = extracted_dir / "toolkit.version"
    if not path.exists() or not path.is_file():
        return {}

    key_map = {
        "package author": "charter",
        "package version": "version",
        "package comment": "notes",
    }
    out: dict[str, str] = {}
    try:
        lines = path.read_text(encoding="utf-8", errors="replace").splitlines()
    except Exception as e:
        log.debug("toolkit.version read failed: %s", e)
        return out
    ...
    for line in lines:
        ...
        mapped_key = key_map.get(raw_key.strip().lower())
        if not mapped_key or mapped_key in out:
            continue
...
def _parse_psarc_manifest_metadata(extracted_dir: Path) -> dict[str, str]:
    """Best-effort metadata extraction from unpacked PSARC JSON manifests."""
    key_map: dict[str, tuple[str, ...]] = {
        "genre": ("SongGenre", "Genre", "songGenre", "genre"),
        "albumArtist": ("AlbumArtistName", "AlbumArtist", "albumArtist", "album_artist", "albumartist"),
        "composer": ("Composer", "Songwriter", "Writer", "composer", "songwriter", "writer"),
        "trackNumber": ("TrackNumber", "SongNumber", "SongOrder", "trackNumber", "tracknumber", "track", "TRCK", "trkn"),
        "discNumber": ("DiscNumber", "discNumber", "discnumber", "disc", "TPOS", "disk"),
        "isrc": ("ISRC", "isrc", "TSRC"),
        "copyright": ("Copyright", "copyright", "TCOP", "cprt"),
        "source": ("Source", "source", "ImportSource", "importSource", "SKU", "Sku", "sku"),
        "platform": ("Platform", "platform"),
        "dlcKey": ("DLCKey", "dlcKey", "dlc_key", "PersistentID", "persistentID", "persistentId"),
        "originalFile": ("OriginalFile", "originalFile", "original_file", "InputFile", "inputFile"),
    }

    out: dict[str, str] = {}

    def collect_from(obj: object) -> None:
        ...
        for canonical, aliases in key_map.items():
            ...
            for alias in aliases:
                value = _meta_text(obj.get(alias))
                if value:
                    out[canonical] = value
                    break
    ...
    for json_file in sorted(extracted_dir.rglob("*.json")):
        ...
        collect_from(data)
        collect_from(data.get("metadata"))
        ...
        if isinstance(entries, dict):
            for entry in entries.values():
                if not isinstance(entry, dict):
                    continue
                collect_from(entry)
                collect_from(entry.get("Attributes"))

        if len(out) == len(key_map):
            break
...
def _wem_to_wav(wem_path: str, out_wav: Path) -> None:
    ...
    # Preferred path: pure-Python WEM extraction, then ffmpeg OGG->WAV.
    ffmpeg = _ffmpeg_cmd()
    if _convert_wem_to_ogg_py and ffmpeg:
        with tempfile.TemporaryDirectory(prefix="s2p_wem_fallback_") as td:
            fallback_ogg = Path(td) / "decoded.ogg"
            try:
                ok = _convert_wem_to_ogg_py(wem_path, str(fallback_ogg))
            except Exception as e:
                ok = False
                py_error = f"python wem_decode raised: {e}"
            else:
                py_error = "python wem_decode could not extract a playable OGG"

            if ok and fallback_ogg.exists() and fallback_ogg.stat().st_size >= 100:
                r2 = subprocess.run(
                    [ffmpeg, "-y", "-i", str(fallback_ogg), str(out_wav)],
                    capture_output=True,
                )
                if r2.returncode == 0 and out_wav.exists() and out_wav.stat().st_size >= 100:
                    return
                py_error = (
                    "fallback OGG->WAV encode failed: "
                    f"{r2.stderr.decode(errors='replace') if r2.stderr else r2.stdout.decode(errors='replace')}"
                )
    elif not _convert_wem_to_ogg_py:
        py_error = "python wem_decode module unavailable"
    else:
        py_error = "ffmpeg not found on PATH for fallback OGG->WAV"

    # Secondary path: native vgmstream direct decode.
    vgmstream_error = "vgmstream-cli not found on PATH"
    vgmstream = _vgmstream_cmd()
    if vgmstream:
        r = subprocess.run([vgmstream, "-o", str(out_wav), wem_path], capture_output=True)
        if r.returncode == 0 and out_wav.exists() and out_wav.stat().st_size >= 100:
            return
        vgmstream_error = (
            "vgmstream-cli failed: "
            f"{r.stderr.decode(errors='replace') if r.stderr else r.stdout.decode(errors='replace')}"
        )

    raise RuntimeError(f"WEM decode failed ({vgmstream_error}; {py_error})")
...
def cleanup_stale_temp_dirs(min_age_seconds: float = 0.0) -> int:
    ...
    - A second feedBack Studio instance can share `tempfile.gettempdir()`
...
def convert_psarc_to_sloppak(
    ...
    try:
        ...
        playable_arrangements = [
            a for a in song.arrangements
            if str(getattr(a, "name", "")).strip().lower() not in _EXCLUDED_ARRANGEMENTS
        ]

        try:
            tones_by_arr = extract_tones_for_song(
                tmp_extract, [a.name for a in playable_arrangements]
            )
            tones_by_arr = {
                name: annotate_tone_block_with_vst(block)
                for name, block in tones_by_arr.items()
        ...
        first = True
        for arr in playable_arrangements:
        ...
        psarc_metadata = _parse_psarc_manifest_metadata(tmp_extract)
        authoring_metadata = _parse_toolkit_authoring_metadata(tmp_extract)
        ...
        metadata_block = dict(psarc_metadata)
        if authoring_metadata:
            metadata_block.update(authoring_metadata)
        metadata_block.setdefault("source", "psarc")
        metadata_block.setdefault("originalFile", psarc_path.name)
        if metadata_block:
            manifest["metadata"] = metadata_block
        if cover_rel:
...
# `stem_separation` manifest block constants. Engine id is stable
# ("demucs" is the only stem separator we currently call).
...
def _probe_local_demucs_runtime() -> list[str]:
    """Return import failures for local Demucs runtime deps.

    Demucs execution in this module relies on these imports in the child
    interpreter bootstrap: demucs, torchaudio, soundfile, numpy.
    """
    problems: list[str] = []
    for module_name in ("demucs", "torchaudio", "soundfile", "numpy"):
        try:
            importlib.import_module(module_name)
        except Exception as e:
            problems.append(f"{module_name}: {type(e).__name__}: {e}")
    return problems


def demucs_available() -> bool:
    return not _probe_local_demucs_runtime()


def _ensure_local_demucs_runtime() -> None:
    problems = _probe_local_demucs_runtime()
    if not problems:
        return
    ...
    raise RuntimeError(
        "Local Demucs dependencies are missing or broken "
        f"({summary}). Install them in the backend environment with "
...
def _run_demucs(audio_path: Path, out_dir: Path, model: str) -> Path:
    ...
    # in desktop builds (lib/sloppak_convert.py → resources/feedBackStudio/lib
    ...
    if r.returncode != 0:
        ...
        if "No module named 'torchaudio'" in tail:
            raise RuntimeError(
                "Local Demucs requires torchaudio. Install backend stem "
                "dependencies with `pip install -r "
                "backend/requirements.txt` and retry."
            )
        raise RuntimeError(
...
def _rewrite_stems_manifest(
    ...
    *,
    ...
    Pass it (only) when stems were produced
...
def _rewrite_lyrics_manifest(
    ...
    *,
    ...
    block (engine / model / version) using the same shape as the
    `stem_separation` metadata for stems. Set it
...
def _maybe_transcribe_lyrics(
    ...
    *,
    ...
    # `stem_separation` metadata pattern. Engine + schema
...
def _maybe_extract_pitch(
    ...
    *,
    ...
    On success writes `vocal_pitch.json` in the renderer-compatible shape:
...
def _resolve_source_mix(source_dir: Path) -> Path | None:
    """Locate the combined-mix audio for a never-split sloppak whose mix isn't
    named ``stems/full.ogg``.

    PSARC→sloppak conversion writes ``stems/full.ogg``, but older / editor-
    written sloppaks name the single combined stem differently (e.g.
    ``stems/audio.mp3``). Read the manifest's ``full`` stem (or the sole stem
    when there's exactly one) and return its on-disk path so those sloppaks can
    still be stem-split. Returns ``None`` when no single source mix is found.
    """
    manifest = None
    for _name in ("manifest.yaml", "manifest.yml"):
        _mp = source_dir / _name
        if _mp.exists():
            try:
                manifest = yaml.safe_load(_mp.read_text(encoding="utf-8")) or {}
            except Exception:
                manifest = None
            break
    ...
    if not isinstance(stems, list):
        return None
    cand = next(
        (s for s in stems if isinstance(s, dict) and s.get("id") == "full"),
        None,
    )
    if cand is None and len(stems) == 1 and isinstance(stems[0], dict):
        cand = stems[0]
    if cand is None:
        return None
    rel = cand.get("file")
    if not isinstance(rel, str) or not rel:
        return None
    # Constrain the manifest-supplied path to the sloppak dir — this path is
    # later read AND unlinked, so a crafted manifest (e.g. file: ../../x) must
    # not escape to read/delete files outside the sloppak.
    try:
...
def _split_in_dir(
    ...
    # The combined-mix file inside stems/ to remove once per-instrument stems
    # exist (full.ogg, or whatever the manifest names the source mix).
    source_mix_in_stems: Path | None = None
    if separation_audio is None:
        if full_ogg.exists():
            separation_audio = full_ogg
            source_mix_in_stems = full_ogg
        else:
            # No stems/full.ogg — fall back to the manifest's source mix (e.g.
            # an older sloppak's stems/audio.mp3). Demucs accepts any common
            # audio format, so no pre-transcode is needed.
            resolved = _resolve_source_mix(source_dir)
            if resolved is None:
                raise FileNotFoundError(
                    f"{full_ogg} not found and the manifest has no single "
                    "source mix — run PSARC conversion first or add "
                    "stems/full.ogg."
                )
            separation_audio = resolved
            source_mix_in_stems = resolved
    ...
    # Remove the original combined mix now that per-instrument stems exist —
    # `full_ogg` above covers the conventional case; this also drops a
    # differently-named source mix (e.g. an older sloppak's stems/audio.mp3).
    if source_mix_in_stems is not None and source_mix_in_stems != full_ogg:
        source_mix_in_stems.unlink(missing_ok=True)
    # Stamp `stem_separation` metadata: which engine /
