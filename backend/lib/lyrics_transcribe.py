...
2. `min_word_score` post-filter — WhisperX's word alignment emits a
   ...
   from the output. The default threshold is 0.35.
"""
...
# syllable in the sloppak output. Set to 3.0s because lower thresholds
# produce short-burst phrasing on
...
def _pick_compute_type(device: str) -> str:
    """Use float16 on CUDA and int8 on CPU.
...
def transcribe_vocals_local(
    ...
    *,
    ...
    uses the platform default selected by `_pick_compute_type` when `None`."""
    try:
        import whisperx
