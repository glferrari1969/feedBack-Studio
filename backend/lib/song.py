...
def arrangement_string_count(arr: Arrangement) -> int:
    ...
    * Empty arrangement named "Lead" (tuning len 6) →
    ...
    Arrangement selection should not rely on arrangement-name
...
def _guess_arrangement_label(path: Path) -> str:
    low = path.stem.lower()
    if "bass" in low:
        return "Bass"
    if "rhythm" in low:
...
def _guess_sng_platform(path: Path):
    try:
        from rs_sng_xml import Platform  # type: ignore
    except Exception:
        return None
...
def _convert_sng_to_xml(extracted_dir: str):
    """Best-effort conversion of RS SNG/notechart files into arrangement XML.

    Official PSARC packages often carry playable charts only as encrypted
    ``.sng``/``.notechart`` files. `load_song` expects arrangement XML, so
    missing conversion makes conversion jobs fail with "no playable
    arrangements found". This helper keeps the conversion local and optional:
    errors on one chart never abort the full song load.
    """
    try:
        from rs_sng_xml import Platform, sng_to_xml  # type: ignore
    except Exception as e:
        log.debug("rs_sng_xml unavailable, skipping SNG conversion: %s", e)
        return None

    root = Path(extracted_dir)
    converted = 0

    notechart_files = sorted(root.rglob("*.sng")) + sorted(root.rglob("*.notechart"))
    for sng_path in notechart_files:
        stem_low = sng_path.stem.lower()
        if any(tag in stem_low for tag in ("vocals", "jvocals", "showlights")):
            continue

        xml_path = sng_path.with_suffix(".xml")
        ...
        try:
            ...
            log.debug("failed reading notechart %s: %s", sng_path.name, e)
            continue

        arrangement = _guess_arrangement_label(sng_path)
        preferred = _guess_sng_platform(sng_path) or Platform.PC
        fallback = Platform.MAC if preferred == Platform.PC else Platform.PC

        converted_here = False
        for platform in (preferred, fallback):
            try:
                xml_bytes = sng_to_xml(data, platform=platform, arrangement=arrangement)
        ...
        if not converted_here:
            log.debug("failed converting notechart %s with both platform keys", sng_path.name)

    if converted:
        log.info("converted %d notechart files to XML", converted)
    return None
